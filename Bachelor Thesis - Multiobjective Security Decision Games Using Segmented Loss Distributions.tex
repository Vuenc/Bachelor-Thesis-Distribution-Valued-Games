...
%              + \lintegral{[1, x_0)\cap \set{f = g}}{t^k(g(t) - f(t))}{\lambda(t)}
    ...
    \begin{defn}
        ...
        \end{gather*}
    \end{defn}
    $\leqRlex$ of course is essentially the usual lexicographic order, but starting comparisons from the right instead of from the left.
...
%                = \biggpars{\sum_{k \geq 1} (c-1) \frac{1-a_k}{c-1} * \1_{[2-a_k, 2-a_{k+1})}} + \1_{[2, \infty)}
            ...
            \end{multline*}
            In particular, $F_c$ has the property to be upper-bounded by the line $x \mapsto x-1$ on [1, 2], touching it exactly at its discontinuity points, i.e. $F_c(2-a_k) = 1-a_k$.
            We can use this to construct two distributions with infinitely alternating distribution functions, as shown by a graphical proof, see Figure \ref{fig:distributionsGeometricSeriesCounterexample-2-3}:
%            $F_c$ “jumps” to the diagonal at every point $2-\frac{1}{c^k}$, but stays below it everywhere else.
            If we choose $c, d$ such that the sets $\set{2 - \frac{1}{c^k} \mid k \in \N}, \set{2 - \frac{1}{d^k} \mid k \in \N}$ are disjoint,
            then $F_c, F_d$ infinitely alternate, since at every jump, one distribution function overtakes the other.
            ...
            We adjust the probability of $\tilde{s}_i$ by a factor $c_i < 1$, i.e. $g(\tilde{s}_i) = c_i f(s_i)$, chosen such that $c_i \geq \frac{s_i}{\tilde{s_i}}$ (ensuring $\tilde{P}$ has greater moments than $P$), $c_i \geq 1-\frac{1}{2^i}$ (ensuring the distribution functions alternate), and $c_i \geq \frac{f(s_{i+1})}{f(s_i)}$ (ensuring $g$ 
