...
%                ((p_{1,1},\dots,p_{1,\abs{S_1}}),\dots,(p_{n,1},\dots,p_{n,\abs{S_n}})
    ...
    We will introduce some more concepts and see a proof of this theorem in section \ref{sec:existenceOfMixedStrategyEquilibria}.
...
%    Substituting
    ...
    \begin{proof}[Proof of \ref{thm:existenceOfMixedStrategyEquilibria}]
        ...
        In the context of this proof, let each player $k$ have $m_k$ different pure strategies and set $m := m_1 + \dots + m_n$.
        We represent mixed strategy profiles as points in $\R^m$: Let $\Delta_k := \set{(p_1, \dots, p_{m_k}) \in \R^{m_k} \mid \sum_{i=1}^{m_k} p_i = 1}$ (the standard $(m_k - 1)$-simplex) and let $\Delta := \bigtimes_{k=1}^n \Delta_k \subseteq \R^m$.
