...
%    Minimal elements with respect to $\leqdom$ are Pareto-minimal; so are elements $x$ where for all $y$, $y \leqpareto x$ implies $x \leqpareto y$.
    ...
    while the terminology “Pareto-optimal” in this context starts to appear only in later sources. %TODO cite
    ...
    To get a better intuition of which Pareto-Nash equilibria a game can have, one can experiment with different weights and visualize the resulting equilibria profiles in a plot where the strategies in the profile are represented as points.
    \autoref{fig:paretoNashEquilibriaShowcase} % TODO figure showing Pareto-Nash equilibria
    shows examples of such plots for randomly generated 2-player games with 3 strategies for each player, and a varying number $m$ of objectives: For each equilibrium profile, the strategies of player 1/2 are represented by a red/green point, projected from the two-dimensional mixed-strategy-simplex in three-dimensional space to the plane.
    The example showcase the possible complexity of the set of Pareto-Nash equilibria:
    While in some games, all equilibria just mix between two of the three strategies, in others some clearly recognizable patterns appear, where for example many equilibrium points concentrate on curves.
...
%        
        ...
        \caption{Different sets of Pareto-Nash equilibria visualized}
    \end{figure}
