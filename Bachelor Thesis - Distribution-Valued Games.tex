...
%    , players can be thought of as neither being able to communicate, nor to make binding agreements.
...
%        Since they have no way to coordinate their actions, both wonder what to do.
...
%        Each row/column represents a strategy by player 1/2,.
...
%                ((p_{1,1},\dots,p_{1,\abs{S_1}}),\dots,(p_{n,1},\dots,p_{n,\abs{S_n}})
    ...
    For this reason, mixed extensions of two-player finite % TODO define finite games
...
%        no player has an incentive to deviate from $s$, since $s_k$ gives the maximal payoff with respect to $s_{-k}$.
    ...
    We are not giving a proof for Kakutani's Theorem, but equipped with it, we are now ready to proof Theorem \ref{thm:existenceOfMixedStrategyEquilibria} that every bimatrix game as an equilibrium in mixed strategies.
...
%    Now that we covered the basic concepts of standard game theory, we are ready to introduce the new theory of games that have probability distributions as payoffs.
    The model of distribution-valued games was first introduced by Stefan Rass % TODO cite
...
%            Note that this is a weighted sum of Borel probability measures, and the result is again a probability measure on $(\R, \B)$.
    ...
    \newcommand{\DP}{\D} % Shortcut for the set of distributions on the reals
...
%            \geq - \lintegral{[1, x_0]}{x^k}{P_1(x)}
    ...
    \begin{thm}
        Let $P_1, P_2 \in \Dgeqone \cap L$ be absolutely continuous, with densities $f, g$.
        \begin{enumerate}
            \item 
