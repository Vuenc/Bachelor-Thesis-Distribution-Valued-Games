...
%            Note that this is a weighted sum of Borel probability measures, and the result is again a probability measure on $(\R, \B)$.
    ...
    In this section we build on the framework from the previous section to introduce distribution-valued games.
...
%        i.e. $\hat{G}^{(\R)}$ has the same Nash equilibria as $\hat{G}^{(\DP)}_{\leqE}$.
    ...
    Antisymmetry can also not hold on all of $M$: there exist examples of different distributions in $\M$ which have equal moment sequences (an example is given in \cite[Example 3.15]{bib:romanoCounterexamplesInProbability}).
...
%        Since the growth of $x_0^k$ dominates the growth of $x_1^k$, this shows that the difference is eventually positive.
        ...
        \end{multline*}
        We define $\mu \coloneqq (P_2 - P_1)\vert_{\B \cap [x_0, \infty)}$ as the difference ($P_2 - P_1)$ restricted to the Borel sub-$\sigma$-algebra on $[x_0, \infty)$. By \eqref{eq:p2DominatesP1LeftOfX0}, $\mu$ is non-negative and therefore a measure, and by \eqref{eq:p2StrictlyDominatesP1OnB0}, it is not the zero measure.
        We can now estimate
        \begin{gather*}
...
%    \end{proof}
    ...
    \begin{defn}
        ...
        \end{gather*}
    \end{defn}
    $\leqRlex$ is similar to the usual lexicographic order, but starts comparing from the right instead of from the left \footnote{There seems to be no consistent name for this ordering in the literature; The term \emph{reflected lexicographic order} is supported by \cite{bib:oeisRefLexOrder}.}. Just as the lexicographic order, it is antisymmetric and total. 
    ...
    While finite support allows us to turn the sufficient condition for the tail order into an equivalent condition, this does not work in the more general case of distributions in $\Dgeqone \cap \M$ which are either absolutely continuous, or discrete with countably infinite support.
%    , Theorems \ref{thm:acTailOrderSuffConditions} and \ref{thm:discreteTailOrderSuffConditions} only give sufficient conditions.
    To show that none of sufficient conditions from \ref{thm:acTailOrderSuffConditions} and \ref{thm:discreteTailOrderSuffConditions} \ref{item:discreteTailOrder-DominatingPointSuffCondition} can be turned into equivalences, we construct several counterexamples.
%    can not be made into equivalences, as the following counter-examples show. \cite{bib:rassGameRiskManagI} originally claimed an equivalence (meanwhile corrected) and assumed distributions with bounded support and continuous density functions, which is why we give counterexamples satisfying these properties.
    \begin{ex}~
