...
%            \settowidth{\baselineskip}{0.4cm}
    ...
    \begin{abstract}
        ...
        In particular, the tail order is shown not to be a total ordering on a certain set of distributions. It is also shown that not all tail-ordered games have mixed-strategy Nash equilibria, and in fact almost all tail-ordered games with finitely-supported payoff distributions only have a Nash equilibrium if they have a pure Nash equilibrium.
        
        The thesis subsequently extends an idea from \cite{bib:tweakableStochasticOrders} and proposes a different solution concept for distribution-valued games. This concept is based on constructing multi-objective real-valued games from distribution-valued games by segmenting their payoff distributions.
    \end{abstract}
...
%        A \emph{stochastic order} $\dleq$ is a preorder on $\DP$.
    ...
    \autoref{fig:distributionValuedGame} shows an example of a distribution-valued game.
    
    \begin{figure}
        \centering
        \begin{tabular}{c|c|c|}
            &        $b_1$        &        $b_2$        \\ \hline
            \raisebox{45pt}{$a_1$} & \shortstack{\adjustbox{margin=5pt 5pt 5pt 8pt}{\includegraphics[width=0.40\textwidth]{Pictures/example-distval-game-F1}}  \\ $\scriptstyle U([0, 5])$ \\~ } & \shortstack{\adjustbox{margin=5pt 5pt 5pt 8pt}{\includegraphics[width=0.40\textwidth]{Pictures/example-distval-game-F2}}  \\ $\scriptstyle \text{Half-Normal distribution}$ \\~ } \\\hline
        ...
        \end{tabular}
        \caption{A distribution-valued 2x2-game with both AC and discrete distributions as payoffs (represented by their distribution functions).}
...
%        \end{subfigure}
    ...
    % TODO MGSS should be talked about before
